...
- `obj.ptr`, if present, must be a `Union{Ptr{Nothing},SignatureStruct}`
"""
...
function Base.getproperty(obj::AbstractGitObject, name::Symbol)
    ...
    elseif name === :ptr
        return getfield(obj, :ptr)::Union{Ptr{Nothing},SignatureStruct}
    else
...
function Base.getproperty(obj::GitObject, name::Symbol)
    ...
    elseif name === :ptr
        return getfield(obj, :ptr)::Ptr{Nothing}
    else
        return getfield(obj, name)
    end
end
