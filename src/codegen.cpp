...
extern "C" void jl_write_coverage_data(const char *output)
{
    if (output) {
        ...
        std::string stm;
        llvm::raw_string_ostream(stm) << "." << jl_getpid() << ".cov";
...
{
    std::string stm;
    llvm::raw_string_ostream(stm) << "." << jl_getpid() << ".mem";
        ...
        if (mi == ctx.linfo) {
            ...
            if (ci != jl_nothing && codeinst->invoke != jl_fptr_sparam) { // check if we know we definitely can't handle this specptr
                if (codeinst->invoke == jl_fptr_const_return) {
                    ...
                    if (need_to_emit) {
                        llvm::raw_string_ostream(name) << (specsig ? "j_" : "j1_") << name_from_method_instance(mi) << "_" << globalUnique++;
...
static Function *emit_tojlinvoke(jl_code_instance_t *codeinst, Module *M, jl_codegen_params_t &params)
{
    ...
    std::string name;
    llvm::raw_string_ostream(name) << "tojlinvoke" << globalUnique++;
    ...
    jl_unionall_t *unionall_env, jl_svec_t *sparam_vals, jl_array_t **closure_types)
{
    ...
    std::string funcName;
    llvm::raw_string_ostream(funcName) << "jlcapi_" << name << "_" << globalUnique++;
        ...
        jl_codegen_params_t &params)
{
    ...
    llvm::raw_string_ostream funcName(_funcName);
    ...
    if (specsig) { // assumes !va and !needsparams
        ...
        std::string wrapName;
        llvm::raw_string_ostream(wrapName) << "jfptr_" << unadorned_name << "_" << globalUnique++;
