        ...
        bool runtime_lib)
{
    ...
    std::string fname;
    llvm::raw_string_ostream(fname) << "jlplt_" << f_name << "_" << globalUnique++;
...
{
    std::string _msg;
    llvm::raw_string_ostream msg(_msg);
    msg << fname;
    if (n > 0)
        msg << " argument " << n;
    else
...
static jl_cgval_t emit_llvmcall(jl_codectx_t &ctx, jl_value_t **args, size_t nargs)
{
    ...
    if (isString) {
        ...
        std::string ir_name;
        while (true) {
            llvm::raw_string_ostream(ir_name) << (ctx.f->getName().str()) << "u" << globalUnique++;
