        ...
        const std::string &txt)
{
    ...
    jl_codegen_params_t::SymMapGV::iterator pooledval;
    bool inserted;
    std::tie(pooledval, inserted) = emission_context.stringConstants.insert(
            std::make_pair(ctxt, nullptr));
    Module *M = jl_builderModule(irbuilder);
    StringRef localname;
    std::string ssno;
    GlobalVariable *gv = nullptr;
    if (inserted) {
        llvm::raw_string_ostream(ssno) << "_j_str" << emission_context.stringConstants.size();
    ...
    if (gv == nullptr)
        gv = get_pointer_to_constant(Data, localname, *M);
    assert(localname == gv->getName());
    assert(Data == gv->getInitializer());
    if (inserted)
        pooledval->second = gv;
